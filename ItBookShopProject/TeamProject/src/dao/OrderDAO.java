...
public OrderDAO() {}
	...
	//mypage
			public ArrayList<OrderBean> getOrderList(String uId) { // 아이디별 주문목록 가져오기
				...
				try {
					String sql = 
							...
							"where tb.order_ID=? order by orderTime desc";
			...
			public OrderBean getDeliveryInfo(int orderDetailNum) {
				System.out.println("OrderDAO - public OrderBean getDeliveryInfo(int orderDetailNum)");
				...
				try {
					String sql = "select *\n"+ 
							"from order_detail join order_tb \n"+ 
							"on order_detail.orderNum = order_tb.orderNum\n"+ 
							"where order_detail.orderDetailCode=?";
					...
					orderBean.setOrderRec(rs.getString("orderRec"));
					};
